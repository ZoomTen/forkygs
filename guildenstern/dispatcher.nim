...
proc suspend(
    server: GuildenServer, sleepmillisecs: int
) {.gcsafe, nimcall, raises: [].} =
  {.gcsafe.}:
    discard workerdatas[server.id].activethreadcount.atomicdec()
    server.log(
      TRACE,
      "suspending thread " & $getThreadId() & " for " & $sleepmillisecs & " millisecs",
    )
    sleep(sleepmillisecs)
    discard workerdatas[server.id].activethreadcount.atomicinc()

proc getSelectorForSocket(
    server: GuildenServer, socket: posix.SocketHandle
): Selector[SocketData] =
  {.gcsafe.}:
    if isNil(workerdatas[server.id].gsselector):
      return nil
    if not workerdatas[server.id].gsselector.contains(socket.int):
      return nil
    return workerdatas[server.id].gsselector

{.warning[Deprecated]: off.}
proc closeSocketImpl(
  ...
  try:
    if not isNil(theselector):
      theselector.unregister(socket.int)
  except:
...
proc restoreRead(
    server: GuildenServer, selector: Selector[SocketData], socketdata: SocketData
) {.inline.} =
  if unlikely(shuttingdown or socketdata.socket == INVALID_SOCKET) or
      not selector.contains(socketdata.socket):
    return
  try:
    ...
    if getCurrentExceptionMsg().startsWith("File exists"):
      ...
      closeSocket(
        server, socketdata.socket, NetErrored,
        "Could not restore Read event to selector",
      )
...
proc workerthreadLoop(server: GuildenServer) {.thread.} =
  {.gcsafe.}:
    ...
    while true:
      ...
      if unlikely(mytail > workerdatas[server.id].head):
        continue
      ...
      handleRead(
        server,
        workerdatas[server.id].queue[mytail].socket,
        workerdatas[server.id].queue[mytail].customdata,
      )
      server.log(TRACE, "handled event at queue position " & $mytail)

      restoreRead(
        server, workerdatas[server.id].gsselector, workerdatas[server.id].queue[mytail]
      )
  if not isNil(server.threadFinalizerCallback):
    server.threadFinalizerCallback()

proc getFlagsImpl(
    server: GuildenServer, socket: SocketHandle
): int {.nimcall, raises: [].} =
  {.gcsafe.}:
    when not defined(nimdoc):
      withLock(workerdatas[server.id].flagLock):
        ...
        if not selector.isNil:
          return getSafelyData(emptySocketData, selector, socket.int).flags
        else:
          return -1
    else:
      discard
...
proc setFlagsImpl(
    server: GuildenServer, socket: SocketHandle, flags: int
): bool {.nimcall.} =
  {.gcsafe.}:
    withLock(workerdatas[server.id].flagLock):
      let selector = server.getSelectorForSocket(socket)
...
proc processEvent(server: GuildenServer, event: ReadyKey) {.gcsafe, raises: [].} =
  {.gcsafe.}:
    ...
    when not defined(nimdoc):
      var socketdata =
        getSafelyData(emptySocketData, workerdatas[server.id].gsselector, fd.int)
    else:
    ...
    if unlikely(Event.Error in event.events):
      if socketdata.isserversocket:
        server.log(ERROR, "server error: " & osErrorMsg(event.errorCode))
      else:
        let cause =
          if event.errorCode.cint in [2, 9]:
            AlreadyClosed
          elif event.errorCode.cint == 14:
            EFault
          elif event.errorCode.cint == 32:
            ConnectionLost
          elif event.errorCode.cint == 104:
            ClosedbyClient
          else:
            NetErrored
    ...
    if unlikely(Event.Read notin event.events):
      try:
        ...
        closeSocketImpl(
          server, fd, NetErrored, "non-read " & $fd & ": " & $event.events
        )
      except:
        discard
      finally:
    ...
    if unlikely(socketdata.isserversocket):
      try:
        ...
        if unlikely(fd.int in [0, INVALID_SOCKET.int]):
          ...
          return
        workerdatas[server.id].gsselector.registerHandle(
          fd.int, {Event.Read}, SocketData(isserversocket: false, socket: fd)
        )
    ...
    if workerdatas[server.id].activethreadcount >
        workerdatas[server.id].maxactivethreadcount:
      sleep(0)
      return
    ...
    if unlikely(workerdatas[server.id].head == QueueSize - 1):
      server.log(
        TRACE,
        "queue head reached the end, stalling for " &
          $(workerdatas[server.id].head - workerdatas[server.id].tail) & " requests",
      )
    ...
    if workerdatas[server.id].tail > workerdatas[server.id].head:
      workerdatas[server.id].tail = workerdatas[server.id].head
...
proc eventLoop(server: GuildenServer) {.gcsafe, raises: [].} =
  ...
  {.gcsafe.}:
    ...
    if server.port > 0:
      server.log(
        INFO,
        "dispatcher " & $server.id & " now listening at port " & $server.port & " using " &
          $workerdatas[server.id].threadpoolsize & " threads",
      )
    else:
      server.log(
        INFO,
        "dispatcher client-server " & $server.id & " now serving, using " &
          $workerdatas[server.id].threadpoolsize & " threads",
      )
  server.started = true
...
proc startListening(server: GuildenServer): bool =
  ...
  try:
    {.gcsafe.}:
      workerdatas[server.id].gsselector.registerHandle(
        portserver.getFd().int, {Event.Read}, SocketData(isserversocket: true)
      )
    when not defined(nimdoc):
...
proc startEventloop(server: GuildenServer) {.thread, gcsafe, nimcall, raises: [].} =
  {.gcsafe.}:
    var workerthreads =
      newSeq[Thread[GuildenServer]](workerdatas[server.id].threadpoolsize)
    try:
      for i in 0 ..< workerdatas[server.id].threadpoolsize:
        createThread(workerthreads[i], workerthreadLoop, server)
  ...
  if slept > 1000 * waitingtime:
    server.log(
      NOTICE,
      "not all threads stopped after waiting " & $waitingtime &
        " seconds. Proceeding with shutdown anyway.",
    )
  else:
...
proc start*(
  ...
  if workerdatas[server.id].maxactivethreadcount == 0:
    workerdatas[server.id].maxactivethreadcount = max(4, countProcessors())
  workerdatas[server.id].threadpoolsize = threadpoolsize.int
  if workerdatas[server.id].threadpoolsize == 0:
  ...
  if QueueSize < workerdatas[server.id].maxactivethreadcount:
    server.log(
      WARN,
      "QueueSize smaller than maxactivethreadcount, increase with -d:QueueSize:number compiler switch",
    )
  ...
  if not createSelector(server):
    return false
  if server.port > 0 and not startListening(server):
    return false
...
proc registerSocket*(
    theserver: GuildenServer, socket: SocketHandle, flags = 0, customdata: pointer = nil
): bool =
  ## Add a socket whose read events will be then dispatched. Useful for servers operating in client mode.
  try:
    workerdatas[theserver.id].gsselector.registerHandle(
      socket.int,
      {Event.Read},
      SocketData(
        isserversocket: false, socket: socket, flags: flags, customdata: customdata
      ),
    )
