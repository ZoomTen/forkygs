...
proc restoreRead(
    server: ptr GuildenServer, selector: Selector[SocketData], socket: int
) {.inline.} =
  var success = true
  var failures = 0
...
proc findSelectorForSocket(
    server: GuildenServer, socket: posix.SocketHandle
): Selector[SocketData] =
  {.gcsafe.}:
    if workerdatas[server.id].gsselector == nil:
      return nil
...
proc closeSocketImpl(
    socketdata: ptr SocketData, cause: SocketCloseCause, msg: string
) {.gcsafe, nimcall, raises: [].} =
  if socketdata.socket.int in [0, INVALID_SOCKET.int]:
    socketdata.server.log(TRACE, "use of invalid socket: " & $socketdata.socket)
    return
  socketdata.server.log(
    DEBUG, "socket " & $cause & ": " & $socketdata.socket & "  " & msg
  )
  if unlikely(socketdata.isserversocket):
    socketdata.server.log(
      DEBUG, "note: closing socket " & $socketdata.socket & " is server socket"
    )
  try:
    if socketdata.server.onclosesocketcallback != nil:
      socketdata.server.onCloseSocketCallback(socketdata, cause, msg)
...
proc processEvent(server: GuildenServer, event: ReadyKey) {.gcsafe, raises: [].} =
  ...
  if unlikely(socketdata.isserversocket):
    try:
      ...
      if fd.int in [0, INVALID_SOCKET.int]:
        ...
        return
      wd.gsselector.registerHandle(
        fd.int,
        {Event.Read},
        SocketData(server: server, isserversocket: false, socket: fd),
      )
  ...
  if unlikely(wd.head == QueueSize - 1):
    server.log(
      TRACE,
      "queue head reached the end, stalling for " & $(wd.head - wd.tail) & " requests",
    )
...
proc dispatchloop(
    serverptr: ptr GuildenServer
) {.thread, gcsafe, nimcall, raises: [].} =
  ...
  try:
    {.gcsafe.}:
      workerdatas[server.id].gsselector.registerHandle(
        portserver.getFd().int,
        {Event.Read},
        SocketData(server: server, isserversocket: true),
      )
    when not defined(nimdoc):
  ...
  {.gcsafe.}:
    var workerthreads =
      newSeq[Thread[ptr GuildenServer]](workerdatas[server.id].threadpoolsize)
    try:
...
proc start*(
  ...
  if workerdatas[server.id].maxactivethreadcount == 0:
    workerdatas[server.id].maxactivethreadcount = countProcessors()
