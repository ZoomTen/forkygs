...
type
  ...
  WebsocketServer* = ref object of HttpServer
    ...
    afterUpgradeCallback*: WsAfterUpgradeCallback
      ## Optional, good for initialization purposes
...
proc recvHeader(): int =
  if ws.socketdata.socket.bytesRecv(ws.request[0].addr, 2) != 2:
    error("no data")
  ...
  if expectedLen > server.bufferlength:
    error("Maximum request size bound to be exceeded: " & $(expectedLen))
...
proc recvFrame() =
  ...
  while true:
    ...
    let ret =
      if ws.requestlen == 0:
        recv(ws.socketdata.socket, addr ws.request[0], expectedLen.cint, 0x40)
      else:
        recv(
          ws.socketdata.socket,
    ...
    if ret == 0:
      (closeSocket(ClosedbyClient, ""); opcode = WsFail; return )
    if ret == -1:
      ...
      wsserver.log(
        WARN,
        "websocket " & $ws.socketdata.socket & " receive error: " & $lastError & " " &
          osErrorMsg(OSErrorCode(lastError)),
      )
      opcode = WsFail
...
proc replyHandshake(): (SocketCloseCause, string) =
  if not readHeader():
    return (ProtocolViolated, "ws header failure")
  if not parseRequestLine():
    return (ProtocolViolated, "ws requestline failure")
...
proc handleWsUpgradehandshake() =
  var (state, errormessage) =
    try:
      ...
      (Excepted, getCurrentExceptionMsg())
  if state != DontClose:
...
proc sendNonblocking(
  ...
  if socket.int in [0, INVALID_SOCKET.int]:
    return (Err, 0)
  let len = text[].len
  if sent == len:
    return (Delivered, 0)
  ...
  if ret < 1:
    if ret == -1:
      ...
      server.log(
        NOTICE,
        "websocket " & $socket.int & " send error: " & $lastError & " " &
          osErrorMsg(OSErrorCode(lastError)),
      )
...
proc closeSocketsInFlight(
    server: WebsocketServer, sockets: seq[posix.SocketHandle], states: seq[State]
): int =
  for i in 0 ..< states.len:
    if states[i].sendstate == Continue:
      server.closeOtherSocket(sockets[i], TimedOut, "")
      withLock(server.sendlock):
        server.sendingsockets.excl(sockets[i])
      result.inc
...
proc send*(
  ...
  if server == nil or delivery.sockets.len == 0:
    return
  ...
  {.gcsafe.}:
    if delivery.message[].len == 0:
      ...
      if delivery.binary:
        createWsHeader(delivery.message[].len, OpCode.Binary)
      else:
        createWsHeader(delivery.message[].len, Opcode.Text)
  var handled = 0
  ...
  while true:
    ...
    if getMonoTime() - start > timeout:
      result +=
        WebsocketServer(server).closeSocketsInFlight(delivery.sockets, delivery.states)
    ...
    if delivery.states[s].sendstate == NotStarted:
      withLock(WebsocketServer(server).sendlock):
        if WebsocketServer(server).sendingsockets.len == MaxParallelSendingSockets:
          server.log(
            INFO,
            "MaxParallelSendingSockets reached, sleeping for " & $(sleepmillisecs) &
              " ms",
          )
          suspend(sleepmillisecs)
          continue
        if WebsocketServer(server).sendingsockets.contains(delivery.sockets[s]):
          continue
        WebsocketServer(server).sendingsockets.incl(delivery.sockets[s])
      var headerstate: SendState
      (headerstate, ret) = WebsocketServer(server).sendNonblocking(
          delivery.sockets[s], addr wsresponseheader
        )
      if headerstate == Delivered:
        ...
        if headerstate == Continue:
          server.closeOtherSocket(delivery.sockets[s], TimedOut)
        result.inc
        handled.inc
        server.log(
    ...
    (delivery.states[s].sendstate, ret) = WebsocketServer(server).sendNonblocking(
...
proc send*(
  ...
  when not compiles(unsafeAddr message):
    {.
      fatal: "posix.send requires taking pointer to message, but message has no address"
    .}
  when not defined(nimdoc) and not defined(gcDestructors):
    {.fatal: "mm:arc or mm:orc required".}
  deli.sockets = sockets
  deli.message = unsafeAddr(message)
  deli.binary = false
  return
    send(WebsocketServer(server), unsafeAddr deli, timeoutsecs, sleepmillisecs) == 0
...
proc handleWsRequest(data: ptr SocketData) {.gcsafe, nimcall, raises: [].} =
  ...
  if (unlikely) ws.socketdata.flags == 0:
    ...
    return
  receiveWs()
  if ws.socketdata.socket == INVALID_SOCKET:
    return # socket failed in flight
  case opcode
  of Ping:
    let pingmsg = ""
    if not server.send(ws.socketdata.socket, pingmsg, 5):
      server.log(
        NOTICE,
        "websocket " & $ws.socketdata.socket & " blocking, could not autoreply to Ping",
      )
  of Close:
    {.gcsafe.}:
      ...
      if not server.send(ws.socketdata.socket, message, 5):
        server.log(
          NOTICE,
          "websocket " & $ws.socketdata.socket &
            " blocking, could not autoacknowledge close",
        )
      if statuscode == "":
        closeSocket(ClosedbyClient, "1005")
      else:
        closeSocket(ClosedbyClient, $(byte(statuscode[1]) + 256 * byte(statuscode[0])))
  of WsFail:
    closeSocket(NetErrored, "Websocket failed")
  else:
    {.gcsafe.}:
