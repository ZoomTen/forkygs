...
from posix import
  SocketHandle, send, recv, EAGAIN, EWOULDBLOCK, Timespec, clock_gettime,
  CLOCK_MONOTONIC
...
when not defined(nimdoc) and not defined(gcDestructors):
  {.fatal: "arc or orc memory manager required, e.g. --mm:atomicArc".}
...
type
  ...
  WebsocketServer* = ref object of HttpServer
    isclient* = false
      ## If a `clientmaskkey` is given in initWebsocketServer, this is set to true
    upgradeCallback*: WsUpgradeCallback
      ## Optional, return true to accept, false to decline a websocket upgrade request
    afterUpgradeCallback*: WsAfterUpgradeCallback
      ## Optional, good for sending the very first websocket message to client
    messageCallback*: WsMessageCallback
      ## Triggered when a message is received. Streaming reads are not supported: message length must be shorter than buffersize.
  ...
  WsDelivery* =
    tuple[
      sockets: seq[posix.SocketHandle],
      message: ptr string,
      binary: bool,
      states: seq[State],
    ]
...
const MaxParallelSendingSockets {.intdefine.} = 500
  ## Id max is reached, operations are ceased temporarily to prevent resource exhaustion
...
when not defined(nimdoc):
  proc `$`(x: SocketHandle): string {.inline.} =
    $(x.cint)
...
template error(msg: string, errorcode = 0) =
  var mesg = msg
  if errorcode > 0:
    mesg.add(": " & $errorcode & " " & osErrorMsg(OSErrorCode(errorcode)))
  if errorcode == 9:
    closeSocket(wsserver, thesocket, AlreadyClosed, mesg)
  elif errorcode == -1:
    closeSocket(wsserver, thesocket, TimedOut, mesg)
  else:
    closeSocket(wsserver, thesocket, ProtocolViolated, mesg)
  opcode = WsFail
  return -1
...
proc bytesRecv(buffer: var string, size: int): int =
  if wsserver.sockettimeoutms == -1:
    return recv(thesocket, addr buffer[0], size, 0)
  var
    received = 0
  ...
  while true:
    ...
    if res == -1:
      ...
      if lastError in [EAGAIN.int, EWOULDBLOCK.int]:
        if not isTimeout(backoff, totalbackoff):
          continue
        else:
          error("websocket " & $thesocket & " stalled sending header", -1)
      else:
        error("websocket " & $thesocket & " ws header receive error", lastError)
    elif res > 0:
      received += res
      if received == size:
        return received
    else:
      error("websocket " & $thesocket & " ws header receive error")

{.push warnings: off.} # HoleEnumConv
proc recvHeader(): int =
  if bytesRecv(ws.request, 2) != 2:
    return
  let b0 = ws.request[0].uint8
  let b1 = ws.request[1].uint8
  opcode = (b0 and 0x0f).Opcode
  if b0[1] or b0[2] or b0[3]:
    error("ws receive protocol error")
...
proc recvFrame() =
  ...
  if opcode == WsFail or expectedlen == 0:
    return
  if expectedLen > wsserver.bufferlength:
    opcode = WsFail
    closeSocket(
      wsserver, thesocket, ProtocolViolated,
      "client tried to offer more data than fits to buffer",
    )
    return
  var
    backoff = initialbackoff
  ...
  while true:
    if shuttingdown:
      (opcode = WsFail; return )
    let ret = recv(
      thesocket,
      addr ws.request[ws.requestlen],
      (expectedLen - ws.requestlen).cint,
      MSG_DONTWAIT,
    )
    if shuttingdown:
      (opcode = WsFail; return )
    ...
    if ret == 0:
      closeSocket(
        ...
        thesocket,
        ClosedbyClient,
        "websocket " & $thesocket & " receive error",
      )
      opcode = WsFail
      return
    elif ret == -1:
      ...
      closeSocket(
        ...
        thesocket,
        cause,
        "ws receive error " & $lastError & ": " & osErrorMsg(OSErrorCode(lastError)),
      )
      return
    else:
      ws.requestlen += ret
      if ws.requestlen == expectedlen:
        return
...
proc maskMessage(mask: string, delivery: ptr WsDelivery) {.inline.} =
  for i in 0 ..< delivery.message[].len:
    delivery.message[i] =
      cast[char](cast[uint8](delivery.message[i]) xor cast[uint8](mask[i mod 4]))
...
proc decodeBase16(str: string): string =
  ...
  for i in 0 ..< result.len:
    result[i] =
      chr((nibbleFromChar(str[2 * i]) shl 4) or nibbleFromChar(str[2 * i + 1]))
...
proc replyHandshake(): (SocketCloseCause, string) =
  ...
  if key == "":
    return (ProtocolViolated, "sec-websocket-key header missing")
  let accept =
    if likely(not isNil(wsserver.upgradeCallback)):
      wsserver.upgradeCallback()
    else:
      true
  if not accept:
    reply(Http400)
    ...
    closeSocket(
      ...
      thesocket,
      CloseCalled,
      "ws upgrade request was not accepted: " & getRequest(),
    )
    return
  when not defined(nimdoc):
    {.gcsafe.}:
...
proc handleWsUpgradehandshake() =
  var (state, errormessage) =
    try:
      ...
      (Excepted, "Reply to websocket upgrade failed")
  if state != DontClose:
...
proc send*(
  ...
  timeoutsecs = 2,
  sleepmillisecs = 10,
): bool {.gcsafe, discardable, raises: [].}
...
proc handleWsRequest*() {.gcsafe, nimcall, raises: [].} =
  ...
  if unlikely(flags == -1):
    ...
    if wsserver.isclient:
      if not readHeader():
        closeSocket(wsserver, thesocket, NetErrored, "Websocket header read failed")
      if not setFlags(wsserver, thesocket, 1):
        closeSocket(
          wsserver, thesocket, NetErrored, " websocket disappeared at hanshake"
        )
        server.log(TRACE, "--end receiving websocket--")
        return
    else:
  ...
  of Ping:
    ...
    if not wsserver.send(thesocket, pingmsg, false, 5):
      wsserver.log(
        NOTICE, "websocket " & $thesocket & " blocking, could not autoreply to Ping"
      )
  of Close:
    {.gcsafe.}:
      ...
      if not wsserver.send(thesocket, message, false, 1):
        wsserver.log(
          INFO, "websocket already closed, could not reply to close handshake"
        )
      else:
        if statuscode == "":
          closeSocket(wsserver, thesocket, ClosedbyClient, "1005")
        else:
          closeSocket(
            wsserver,
            thesocket,
            ClosedbyClient,
            $(byte(statuscode[1]) + 256 * byte(statuscode[0])),
          )
  of WsFail:
    server.log(TRACE, "--end receiving websocket--")
    return
    ...
    {.gcsafe.}:
      if likely(ws.requestlen > 0 and not isNil(wsserver.messageCallback)):
        wsserver.messageCallback()
      server.log(TRACE, "--end receiving websocket--")
...
proc createWsHeader(len: int, code: OpCode, isclient: bool, mask: string) =
  ...
  if len > 125 and len <= 0xFFFF:
    ...
    var len64 = swapBytesBuiltin(len.uint64)
      # AMD64 is little endian, Internet is big endian
...
proc sendNonblocking(
    server: WebsocketServer, socket: posix.SocketHandle, text: ptr string, sent: int = 0
): (SendState, int) =
  ...
  if ret < 1:
    ...
    return (Err, 0)
  if sent + ret == len:
    return (Delivered, ret)
  return (Continue, ret)

proc closeSocketsInFlight(
    server: WebsocketServer,
    sockets: seq[SocketHandle],
    states: seq[State],
    failedsockets: var seq[SocketHandle],
) =
...
proc send*(
  ...
  {.gcsafe.}:
    if delivery.message[].len == 0:
      ...
      createWsHeader(
        delivery.message[].len, Opcode.Close, server.isclient, server.clientmaskkey
      )
    else:
      if server.isclient:
        maskmessage(server.clientmaskkey, delivery)
      if delivery.binary:
        createWsHeader(
          delivery.message[].len, OpCode.Binary, server.isclient, server.clientmaskkey
        )
      else:
        createWsHeader(
          delivery.message[].len, Opcode.Text, server.isclient, server.clientmaskkey
        )
  delivery.states.setLen(delivery.sockets.len)
  for i in 0 ..< delivery.states.len:
    delivery.states[i] = (0, NotStarted)
  var
    handled = 0
    s = -1
    blockedsockets = 0
  ...
  while true:
    ...
    if s == delivery.sockets.len:
      s = -1
      continue
    if delivery.states[s].sendstate in [Delivered, Err]:
      continue
    ...
    if delivery.states[s].sendstate == NotStarted:
      {.gcsafe.}:
        withLock(sendlock):
          if sendingsockets.len == MaxParallelSendingSockets:
            server.log(
              INFO,
              "MaxParallelSendingSockets reached, sleeping for " & $(parallelsleep) &
                " ms",
            )
      ...
      (headerstate, ret) =
        server.sendNonblocking(delivery.sockets[s], addr wsresponseheader)
      if headerstate == Delivered:
        delivery.states[s].sendstate = Continue # only header delivered
      else:
        ...
        if headerstate == Continue:
          server.closeSocket(delivery.sockets[s], TimedOut)
        handled.inc
        server.log(
          TRACE, "wsockets processed: " & $handled & "/" & $delivery.sockets.len
        )
        if handled == delivery.sockets.len:
          break
      continue

    if shuttingdown:
      return
    (delivery.states[s].sendstate, ret) = server.sendNonblocking(
      delivery.sockets[s], delivery.message, delivery.states[s].sent
    )
    if delivery.states[s].sendstate == Continue:
      ...
      if blockedsockets >= delivery.sockets.len - handled:
        server.log(
          DEBUG,
          "all remaining websockets are blocking, suspending for " &
            $(blockedsockets * sleepmillisecs) & " ms",
        )
      ...
      {.gcsafe.}:
        withLock(sendlock):
          if delivery.states[s].sendstate == Err:
            failedsockets.add(delivery.sockets[s])
          sendingsockets.excl(delivery.sockets[s])
      handled.inc
...
proc send*(
  ...
  if unlikely(failedsockets.len > 0):
    server.log(
      INFO, "websocket multisend failed for " & $failedsockets.len & " sockets"
    )
...
proc isMessage*(message: string): bool =
  if ws.requestlen != message.len:
    return false
  for i in countup(0, ws.requestlen - 1):
    if ws.request[i] != ws.request[i]:
      return false
  true
...
proc newWebsocketServer(
  ...
  initWebsocketServer(
    result, upgradecallback, afterupgradecallback, onwsmessagecallback, loglevel
  )
...
proc newWebsocketServer*(
  ...
  result = newWebsocketServer(
    upgradecallback, afterupgradecallback, onwsmessagecallback, loglevel
  )
...
proc newWebsocketServer*(
    upgrade: WsUpgradeCallback = nil,
    afterupgrade: WsAfterUpgradeCallback = nil,
    receive: WsMessageCallback,
    close: OnCloseSocketCallback = nil,
    loglevel = LogLevel.WARN,
): WebsocketServer =
