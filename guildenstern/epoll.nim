...
proc registerHandle*[T](
    s: Selector[T], fd: int | posix.SocketHandle, events: set[Event], data: T
) =
  let fdi = int(fd)
  s.checkFd(fdi)
...
proc selectFast*[T](s: Selector[T]): ReadyKey =
  ...
  while true:
    ...
    if (res.events and EPOLLERR) != 0 or (res.events and EPOLLHUP) != 0 or
        (res.events and EPOLLRDHUP) != 0:
    ...
    if (res.events and EPOLLIN) != 0:
      result.events.incl(Event.Read)
    elif Event.User in pkey.events:
      result.events.incl(Event.User)
