...
type
  PartState* = enum
    ...
    HeaderReady
      ## New part starts. [parseContentDisposition] is your friend here. For accessing other fields, use http.headers
...
proc atBoundary(): bool =
  for i in countdown(multipart.boundary.len - 1, 0):
    if multipart.boundary[i] !=
        multipart.partcache[multipart.partlen - 1 - multipart.boundary.len - 1 + i]:
      return false
  true
...
proc processPart(c: char): PartState =
  ...
  multipart.partlen += 1
  if multipart.partlen < multipart.boundary.len:
    return Completed
  if atBoundary():
    multipart.inheader = true
    return BodyReady
  else:
    if multipart.partlen < multipart.partcache.len:
      return Completed
    return BodyChunk
  ...
  for c in chunk:
    if multipart.inheader:
      ...
      if partstate == Failed:
        ...
        yield (
          BodyChunk,
          multipart.partcache[0 .. multipart.partlen - multipart.boundary.len - 1],
        )
        for i in 0 .. multipart.boundary.len - 1:
          multipart.partcache[i] =
            multipart.partcache[multipart.partlen - multipart.boundary.len + i]
...
proc parseHeader(header: string) =
  ...
  while i < header.len:
    ...
    of ' ':
      if value:
        if current[1].len != 0:
          current[1].add(header[i])
      else:
        current[0].add(header[i])
    of '\l':
      ...
      if value:
        ...
        current[0].add((header[i]).toLowerAscii())
    i.inc
...
iterator receiveParts*(parsepartheaders: bool = true): (PartState, string) {.closure.} =
  ...
  if parsepartheaders:
    for key in originalheaderfields:
      http.headers[key] = ""
...
proc handleMultipartRequest() {.gcsafe, nimcall, raises: [].} =
  ...
  multipart.boundary = "--" & contenttype[30 .. ^1]
    # last boundary's extra -- is just ignored
  if unlikely(multipart.boundary.len > server.bufferlength - 1):
    server.log(ERROR, "bufferlength too small, even part boundary does not fit")
  server.log(
    DEBUG,
...
proc newMultipartServer*(
  ...
  if not fields.contains("content-type"):
