...
proc clientThread(server: GuildenServer) {.thread.} =
  ...
  while true:
    ...
    try:
      {.gcsafe.}:
        ...
        if unlikely(Event.User in event.events):
          break
        if unlikely(Event.Error in event.events):
          closeSocketImpl(
            server,
            SocketHandle(event.fd),
            NetErrored,
            "socket " & $event.fd & ": error " & $event.errorCode,
          )
          continue
        ...
        when not defined(nimdoc):
          client =
            getSafelyData(emptyClient, servers[server.id].clientselector, socket.int)
    ...
    if alreadybeingprocessed == true:
      continue

    server.log(
      TRACE, "handleRead starts for socket " & $socket & " at thread " & $getThreadId()
    )
  ...
  {.gcsafe.}:
    discard servers[server.id].threadpoolsize.atomicDec()
...
proc createSelector(server: GuildenServer): bool =
  ...
  try:
    ...
    when not defined(nimdoc):
      ...
      servers[server.id].serversocket.setSockOpt(
        OptNoDelay, true, level = cint(Protocol.IPPROTO_TCP)
      )
  ...
  {.gcsafe.}:
    try:
      ...
      servers[server.id].serverselector.registerHandle(
        servers[server.id].serversocket.getFd(), {Event.Read}, true
      )
      return true
    except:
...
proc listeningLoop(server: GuildenServer) {.thread, gcsafe, nimcall, raises: [].} =
  ...
  while true:
    try:
      {.gcsafe.}:
        discard servers[server.id].serverselector.selectInto(-1, eventbuffer)
    except:
    ...
    when not defined(nimdoc):
      ...
      try:
        ...
        {.gcsafe.}:
          servers[server.id].clientselector.registerEPOLLETReadHandle(
            newsocket.int, client
          )
      except:
        server.log(ERROR, "selector registerHandle error for socket " & $newsocket)
        continue
      server.log(
        DEBUG, "New socket " & $newsocket & " connected at port " & $server.port
      )
  ...
  {.gcsafe.}:
    ...
    while slept <= 1000 * waitingtime:
      ...
      if servers[server.id].threadpoolsize < 1:
        break
      if slept == 200:
...
proc start*(
  ...
  if port > 0 and not createSelector(server):
    return false
  if not startClientthreads(server):
    return false
  if port > 0:
  ...
  if port == 0:
    server.log(
      INFO,
      "epolldispatcher client-server " & $server.id & " now serving, using " &
        $servers[server.id].threadpoolsize & " threads",
    )
  return not shuttingdown

proc registerSocket*(
    server: GuildenServer, socket: SocketHandle, flags = 0, customdata: pointer = nil
): bool =
  ...
  try:
    ...
    when not defined(nimdoc):
      {.gcsafe.}:
        servers[server.id].clientselector.registerEPOLLETReadHandle(socket.int, client)
